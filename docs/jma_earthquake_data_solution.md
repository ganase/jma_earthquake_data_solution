...
本提案のゴールは、気象庁が公開する地震データを継続的に取得し、後続分析で扱いやすい形に保存し、Tableau による監視・探索分析と、特定地震の前後における Gutenberg-Richter 則（以下、GR則）評価を行える基盤を整備することである。

- 気象庁地震データを 5〜10 分程度の間隔で定期取得する。
- raw データを改変せず保存し、分析向けに normalized / curated 層へ整形する。
- Tableau から接続しやすいファクトテーブルを提供する。
...
- `curated`: Tableau や GR 計算で利用しやすい形へ集約。
...
初期運用では 5〜10 分間隔の取得を基本案とする。速報性よりコストや安定性を優先する場合は、30 分〜1 時間間隔から開始してもよい。
...
- Tableau 用の `fact_earthquake` は、原則として最新 revision のみを参照する。
...
品質チェック結果は運用ログに残し、Tableau の監視ダッシュボードにも表示できる形にする。
...
同じ基準地震に対して複数の窓を保存し、Tableau で比較できるようにする。
...
GR則評価は、防災判断や地震予知の確定根拠ではない。Tableau 上でも「参考指標」「補助情報」であることを明示し、前震・余震というラベルを断定的に表示しない。
...
- `fact_earthquake` を作成し、Tableau で地図・時系列を確認する。
...
- `fact_gr_window` を作成し、Tableau で GR評価ダッシュボードを構築する。
...
この構成では、取得、正規化、GR計算、Tableau 連携を別ディレクトリに分ける。ローカル Codex が後続作業を行う際も、修正対象を絞りやすい。

## 9. 運用上の注意

- 気象庁データは改訂される可能性があるため、raw 層では過去 revision を保持する。
- Tableau で見る最新データと、監査用の履歴データを分けて扱う。
...
3. 5〜10 分間隔で動く取得ジョブを作る。
4. `fact_earthquake` を作成し、Tableau で地図と時系列を確認する。
